...
from datasets.process_data import prepare_source_and_target_rigid_3d
...
class ThreeDMatchDemo(Dataset):
    ...
    def __getitem__(self, item):
        ...
        src_pcd = src_pcd.voxel_down_sample(0.005)
        tgt_pcd = tgt_pcd.voxel_down_sample(0.005)
        src_pcd = np.array(src_pcd.points).astype(np.float32)
        tgt_pcd = np.array(tgt_pcd.points).astype(np.float32)

        # src_pcd = np.loadtxt(self.src_path).astype(np.float32)[::5]
        # tgt_pcd = np.loadtxt(self.tgt_path).astype(np.float32)[::5]

        # src_pcd, tgt_pcd = prepare_source_and_target_rigid_3d(source_filename='data/stanford/bunny.pcd')
        # src_pcd = src_pcd.voxel_down_sample(0.004)
        # tgt_pcd = tgt_pcd.voxel_down_sample(0.005)
        # src_pcd = np.array(src_pcd.points).astype(np.float32)
        # tgt_pcd = np.array(tgt_pcd.points).astype(np.float32)
...
def draw_registration_result(src_raw, tgt_raw, src_overlap, tgt_overlap, src_saliency, tgt_saliency, tsfm, vis):
    ########################################
    # 1. input point cloud
...
def main(config, demo_loader):
    ...
    with torch.no_grad():
        ...
        for _ in range(1000):
            ...
            vis = draw_registration_result(src_raw, tgt_raw, src_overlap, tgt_overlap, src_saliency, tgt_saliency, tsfm, vis)
        if vis:
            for v in vis:
                v.destroy_window()
