...
class Dataset:
    def __init__(self):
        ...
        self.instances = ['human_models/head_models/model_women/']
        ...
        print('Total',
              (
                  ...
                  len(self.angles_cutoff_along) if len(self.angles_cutoff_along) > 0 else 1 *
                                                                                          len(self.plane_sizes) if len(
                      self.plane_sizes) > 0 else 1 *
                                                 len(self.Gaussian_sigma_factor) if len(
                      self.Gaussian_sigma_factor) > 0 else 1 *
                                                           self.n_move
...
class Writer(Dataset):
    ...
    def __exe_all(self, sources, output_dir_path, index_start_sources=0, index_end_sources=-1):
        ...
        for index_source, source in enumerate(sources[index_start_sources:index_end_sources]):
            ...
            if not instance_cur or not instance_cur == source['instance']:  # notice if instance change
                ...
                print('     ', id_pc_saving, 'iter: Working on', source['instance'], source['pc_artificial'],
                      ' range from',
    ...
    def __reg_pc(self, model_file_paths, datas_file_paths, poses_file_paths, instances):
        sources = []
        for model_file_path, datas_file_path, pose_file_paths, instance in zip(model_file_paths, datas_file_paths,
                                                                               poses_file_paths, instances):
            for data_file_path, pose_file_path in zip(datas_file_path, pose_file_paths):
                tf_init = np.eye(4)
                pose_model_in_target = np.loadtxt(pose_file_path)
                assert pose_model_in_target.shape == (4, 4), 'Expect pose matrix to be 3x3, but get ' + str(
                    pose_model_in_target.shape) + ' instead'
    ...
    def __load_pc(self, source, flag_show=False):
        ...
        if source['instance'] in self.instances_2_scale:
            ...
            size_origin = np.linalg.norm(
                bound_max - bound_min)  # here we use second order norm of range in every direction
    ...
    def __exe_add_plane(self, source, flag_show=True):
        ...
        # make a empty shadow
        mask = np.logical_or(y < - rg[plane_axis[0]] / 8,
                             np.logical_or(x < - rg[plane_axis[0]] / 4, x > rg[plane_axis[0]] / 4))
        x, y = x[mask], y[mask]
